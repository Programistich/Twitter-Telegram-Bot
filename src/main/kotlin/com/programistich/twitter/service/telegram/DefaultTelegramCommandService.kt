...
class DefaultTelegramCommandService(
    ...
    override fun helpCommand(message: Message) {
        ...
        telegramExecutorService.sendTextMessage(
            chatId,
            """
                Этот бот предназначен для отслеживания людей(пока что поставленные лайки) в твиттере
                • Отправьте ссылку на твит и бот переведет его вам(работает только с правами админа)
                • /get %ссылка_на_твит% - переведенный твит
                • /stock - состояние акций Tesla
                • /new %юзернейм_человека_из_твиттера% - подписка на лайки от конкретного человека 
