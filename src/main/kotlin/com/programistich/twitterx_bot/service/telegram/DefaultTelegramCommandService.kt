...
import com.programistich.twitterx_bot.service.db.DatabaseTwitterUserService
import com.programistich.twitterx_bot.service.stocks.StocksService
import com.programistich.twitterx_bot.service.twitter.TwitterClientService
...
class DefaultTelegramCommandService(
    ...
    override fun newTwitterUserCommand(message: Message, username: String?) {
        ...
        val twitterAccount: TwitterAccount? = if (existUsernameInTwitterDb) {
        ...
        twitterAccount.lastLikeId = twitterClientService.lastLikeTweetByUsername(username).id
        chat.twitterUsers.add(twitterAccount)
        twitterAccount.chats.add(chat)
    ...
    override fun helpCommand(message: Message) {
        ...
        telegramExecutorService.sendTextMessage(
            chatId,
            """
                Этот бот - пет/проект @programistich для освежевания деятельности Илона Маска в Твиттере 
                и вытекающими возможностями, о котором сказано ниже 
                • Отправьте ссылку на твит и бот переведет его вам(в чате работает только с правами админа)
                • /get %ссылка_на_твит% - бот переведет вам твит(аналог 
                • /stock - состояние акций Tesla
                • /new %юзернейм_человека_из_твиттера% - подписка(отслеживания) на лайки от конкретного человека 
